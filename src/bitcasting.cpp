...
namespace bitcasting {
    ...
    template<typename T>
    constexpr T from_ne_bytes(
            Array<uint8_t, sizeof(T) / sizeof(uint8_t)> value) noexcept {
        return *(T *) &value;
    }
    ...
    Array<uint8_t, N>
    bswap(Array<uint8_t, N> value) noexcept {
        Array<uint8_t, N> out{};
    ...
    template<typename T>
    T dangerous_bswap(T t) {
        return from_ne_bytes<T>(
                bswap(to_ne_bytes(t)));
    }
